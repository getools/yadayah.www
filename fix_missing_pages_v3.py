#!/usr/bin/env python3
"""
Fix remaining translations with missing page numbers.

Uses neighbor-based disambiguation: for translations with duplicate text that
appear on multiple pages, uses the page numbers of surrounding translations
to determine the correct page.

Also tries matching with shorter and ASCII-only snippets for hard cases.
"""

import sys
import os
import re
import logging
import tempfile
import subprocess
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent))

from parse_word_translations import (
    extract_translations_from_doc,
    setup_logging,
    DEFAULT_DIRECTORY,
)
from docx import Document

try:
    import psycopg2
except ImportError:
    print("ERROR: psycopg2 not installed")
    sys.exit(1)

try:
    from dotenv import load_dotenv
except ImportError:
    print("ERROR: python-dotenv not installed")
    sys.exit(1)


def get_page_texts(doc_path: str, timeout: int = 300) -> dict:
    """
    Navigate page-by-page via COM GoTo and collect text from each page.
    Returns {footer_page_number: [text_chunk1, text_chunk2, ...], ...}
    """
    out = tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False, encoding='utf-8')
    out.close()

    vbs_content = f'''
Dim word, fso, outputFile
Set fso = CreateObject("Scripting.FileSystemObject")
Set word = CreateObject("Word.Application")
word.Visible = False
word.DisplayAlerts = 0

Set outputFile = fso.CreateTextFile("{out.name}", True, True)

WScript.StdErr.WriteLine "Opening document..."
Dim doc
Set doc = word.Documents.Open("{doc_path}", , True)

Dim totalPages
totalPages = doc.ComputeStatistics(2)
WScript.StdErr.WriteLine "Opened, pages: " & totalPages

Dim p, footerPage, pageText
Dim rng, rngEnd

For p = 1 To totalPages
    On Error Resume Next

    Set rng = doc.GoTo(1, 1, p)

    If p < totalPages Then
        Set rngEnd = doc.GoTo(1, 1, p + 1)
        rng.End = rngEnd.Start
    Else
        rng.End = doc.Content.End
    End If

    footerPage = rng.Information(1)
    pageText = rng.Text

    If Len(pageText) > 4000 Then
        pageText = Left(pageText, 4000)
    End If

    pageText = Replace(pageText, vbCr, " ")
    pageText = Replace(pageText, vbLf, " ")
    pageText = Replace(pageText, vbTab, " ")
    pageText = Replace(pageText, "|", " ")

    If Err.Number = 0 Then
        outputFile.WriteLine footerPage & "|" & pageText
    End If
    On Error GoTo 0
Next

doc.Close False
outputFile.Close
word.Quit
WScript.StdErr.WriteLine "Done - " & totalPages & " pages"
'''

    vbs_file = tempfile.NamedTemporaryFile(mode='w', suffix='.vbs', delete=False, encoding='utf-8')
    vbs_file.write(vbs_content)
    vbs_file.close()

    try:
        result = subprocess.run(
            ["cscript", "//NoLogo", vbs_file.name],
            capture_output=True, text=True, timeout=timeout
        )

        if result.stderr:
            for line in result.stderr.strip().split('\n'):
                if line.strip():
                    logging.info(f"  VBS: {line.strip()}")

        if result.returncode != 0:
            logging.warning(f"  VBS exited with code {result.returncode}")

        # Return ordered list of (page, text) tuples to preserve page sequence
        page_entries = []
        if os.path.exists(out.name):
            with open(out.name, 'r', encoding='utf-16', errors='replace') as f:
                for line in f:
                    line = line.strip()
                    if not line or line.startswith('\ufeff'):
                        line = line.lstrip('\ufeff').strip()
                    if not line:
                        continue
                    pipe = line.find('|')
                    if pipe > 0:
                        try:
                            page = int(line[:pipe])
                            text = line[pipe+1:]
                            page_entries.append((page, text))
                        except ValueError:
                            pass

        return page_entries

    except subprocess.TimeoutExpired:
        logging.warning(f"  COM timed out after {timeout}s")
        try:
            subprocess.run(["powershell", "-Command",
                "Stop-Process -Name WINWORD -Force -ErrorAction SilentlyContinue"],
                capture_output=True, timeout=10)
        except:
            pass
        return []
    except Exception as e:
        logging.warning(f"  COM failed: {e}")
        return []
    finally:
        for fp in [vbs_file.name, out.name]:
            try:
                os.unlink(fp)
            except:
                pass


def norm(text: str) -> str:
    """Normalize text for comparison."""
    return re.sub(r'\s+', ' ', text).strip()


def find_all_pages_for_text(para_text: str, page_entries: list) -> list:
    """
    Find ALL pages that contain the given paragraph text.
    Returns list of page numbers.
    """
    normed = norm(para_text)
    if len(normed) < 10:
        return []

    matching_pages = []
    for snippet_len in [80, 50, 30]:
        if len(normed) < snippet_len:
            snippet = normed
        else:
            snippet = normed[:snippet_len]

        for page, text in page_entries:
            if snippet in norm(text):
                if page not in matching_pages:
                    matching_pages.append(page)

        if matching_pages:
            return matching_pages

    # Try ASCII-only matching (strip non-ASCII)
    ascii_text = re.sub(r'[^\x20-\x7e]', '', normed)
    if len(ascii_text) >= 20:
        snippet = ascii_text[:50]
        for page, text in page_entries:
            ascii_page_text = re.sub(r'[^\x20-\x7e]', '', norm(text))
            if snippet in ascii_page_text:
                if page not in matching_pages:
                    matching_pages.append(page)

    return matching_pages


def main():
    setup_logging(verbose=False)
    load_dotenv()

    logging.info("Fix Missing Page Numbers v3 (neighbor-based disambiguation)")
    logging.info("=" * 60)

    conn = psycopg2.connect(
        host=os.getenv('POSTGRES_HOST', 'localhost'),
        port=os.getenv('POSTGRES_PORT', '5432'),
        user=os.getenv('POSTGRES_USER', 'postgres'),
        password=os.getenv('POSTGRES_PASSWORD'),
        database=os.getenv('POSTGRES_DB', 'yada'),
    )
    cur = conn.cursor()

    # Get books with missing pages
    cur.execute("""
        SELECT translation_book, COUNT(*) as missing
        FROM translation WHERE translation_page IS NULL
        GROUP BY translation_book ORDER BY missing DESC
    """)
    missing_books = cur.fetchall()
    total_missing = sum(row[1] for row in missing_books)
    logging.info(f"{total_missing} missing pages across {len(missing_books)} books\n")

    doc_dir = Path(DEFAULT_DIRECTORY)
    total_fixed = 0
    total_neighbor_fixed = 0

    for book_name, missing_count in missing_books:
        doc_path = doc_dir / f"{book_name}.docx"
        if not doc_path.exists():
            logging.warning(f"  File not found: {doc_path}")
            continue

        file_size_mb = doc_path.stat().st_size / (1024 * 1024)
        logging.info(f"{book_name} ({file_size_mb:.1f}MB, {missing_count} missing)")

        # Step 1: Extract translations to get para_idx for missing ones
        translations = extract_translations_from_doc(doc_path)

        # Step 2: Get ALL translations from DB for this book (with pages)
        cur.execute("""
            SELECT translation_id, translation_text_word, translation_page
            FROM translation
            WHERE translation_book = %s
            ORDER BY translation_id
        """, (book_name,))
        all_db = cur.fetchall()
        db_by_id = {row[0]: {'text_word': row[1], 'page': row[2]} for row in all_db}
        missing_ids = [row[0] for row in all_db if row[2] is None]

        # Step 3: Map missing translations to their paragraph indices
        missing_para_indices = {}
        for t in translations:
            text_word = t.get('text_word', '')
            para_idx = t.get('_para_idx')
            for tid in missing_ids:
                if tid not in missing_para_indices and db_by_id[tid]['text_word'] == text_word:
                    missing_para_indices[tid] = para_idx
                    break

        if not missing_para_indices:
            logging.warning(f"  Could not match any missing translations to paragraphs")
            continue

        # Step 4: Get python-docx paragraph texts
        doc = Document(str(doc_path))
        pdx_texts = {}
        for tid, pidx in missing_para_indices.items():
            if pidx < len(doc.paragraphs):
                pdx_texts[pidx] = doc.paragraphs[pidx].text

        # Step 5: Get page texts from COM (use clean copy)
        subprocess.run(["powershell", "-Command",
            "Stop-Process -Name WINWORD -Force -ErrorAction SilentlyContinue"],
            capture_output=True, timeout=10)

        timeout = int(120 + file_size_mb * 30)
        page_entries = []

        for attempt_label in ["clean copy", "original"]:
            if attempt_label == "clean copy":
                try:
                    clean_dir = tempfile.mkdtemp()
                    clean_path = os.path.join(clean_dir, "clean.docx")
                    Document(str(doc_path)).save(clean_path)
                    attempt_path = clean_path
                except Exception as e:
                    logging.warning(f"  Clean copy failed: {e}")
                    continue
            else:
                attempt_path = str(doc_path)

            logging.info(f"  COM page-by-page ({attempt_label}, timeout {timeout}s)...")
            page_entries = get_page_texts(attempt_path, timeout=timeout)

            if attempt_label == "clean copy":
                try:
                    os.unlink(attempt_path)
                    os.rmdir(os.path.dirname(attempt_path))
                except:
                    pass

            if page_entries:
                unique_pages = len(set(p for p, _ in page_entries))
                logging.info(f"  Got {unique_pages} unique pages from COM")
                break
            else:
                logging.warning(f"  COM failed ({attempt_label})")

        if not page_entries:
            logging.warning(f"  Could not get page texts for {book_name}")
            continue

        # Step 6: Build ordered list of translation IDs for neighbor lookup
        ordered_ids = [row[0] for row in all_db]

        # Step 7: Match each missing translation with neighbor disambiguation
        fixed = 0
        neighbor_fixed = 0
        for tid, pidx in missing_para_indices.items():
            para_text = pdx_texts.get(pidx, '')
            if not para_text:
                continue

            matching_pages = find_all_pages_for_text(para_text, page_entries)

            if len(matching_pages) == 1:
                # Unique match
                page = matching_pages[0]
            elif len(matching_pages) > 1:
                # Multiple pages contain this text - use neighbors to disambiguate
                idx_in_list = ordered_ids.index(tid)

                # Find nearest previous translation with a page number
                prev_page = None
                for i in range(idx_in_list - 1, -1, -1):
                    p = db_by_id[ordered_ids[i]]['page']
                    if p is not None:
                        prev_page = p
                        break

                # Find nearest next translation with a page number
                next_page = None
                for i in range(idx_in_list + 1, len(ordered_ids)):
                    p = db_by_id[ordered_ids[i]]['page']
                    if p is not None:
                        next_page = p
                        break

                # Pick the matching page that's closest to the neighbor range
                page = None
                if prev_page is not None and next_page is not None:
                    # Must be between prev and next page (inclusive)
                    candidates = [p for p in matching_pages if prev_page <= p <= next_page]
                    if len(candidates) == 1:
                        page = candidates[0]
                    elif len(candidates) > 1:
                        # Pick the one closest to the midpoint
                        mid = (prev_page + next_page) / 2
                        page = min(candidates, key=lambda p: abs(p - mid))
                    else:
                        # Relax: pick closest to prev_page
                        page = min(matching_pages, key=lambda p: abs(p - prev_page))
                elif prev_page is not None:
                    # Pick the matching page closest to prev
                    page = min(matching_pages, key=lambda p: abs(p - prev_page))
                elif next_page is not None:
                    page = min(matching_pages, key=lambda p: abs(p - next_page))
                else:
                    page = matching_pages[0]

                if page is not None:
                    neighbor_fixed += 1
            else:
                page = None

            if page is not None:
                cur.execute("""
                    UPDATE translation SET translation_page = %s
                    WHERE translation_id = %s AND translation_page IS NULL
                """, (page, tid))
                if cur.rowcount > 0:
                    fixed += 1
                    # Update our local cache too
                    db_by_id[tid]['page'] = page

        conn.commit()
        total_fixed += fixed
        total_neighbor_fixed += neighbor_fixed
        logging.info(f"  Fixed {fixed}/{missing_count} ({neighbor_fixed} via neighbor disambiguation)")

    # Final check
    cur.execute("SELECT COUNT(*) FROM translation WHERE translation_page IS NULL")
    still_missing = cur.fetchone()[0]

    logging.info(f"\n{'=' * 60}")
    logging.info(f"SUMMARY")
    logging.info(f"Total fixed: {total_fixed}/{total_missing}")
    logging.info(f"  Via neighbor disambiguation: {total_neighbor_fixed}")
    logging.info(f"Still missing: {still_missing}")

    if still_missing > 0:
        cur.execute("""
            SELECT translation_book, COUNT(*) as missing
            FROM translation WHERE translation_page IS NULL
            GROUP BY translation_book ORDER BY missing DESC
        """)
        for book, count in cur.fetchall():
            logging.info(f"  {book}: {count}")

    cur.close()
    conn.close()
    logging.info("Done.")


if __name__ == "__main__":
    main()
