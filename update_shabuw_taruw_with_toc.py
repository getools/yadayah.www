"""
Update Shabuw'ah and Taruw'ah including TOC hyperlinks.
"""
...
def process_runs_in_element(element, stats):
    """Process runs in any XML element (including hyperlinks)."""
    # Find all w:r (run) elements recursively
    ...
    for run_elem in runs:
        ...
        for t_elem in t_elems:
            if t_elem.text:
                new_text = apply_replacements(t_elem.text)
                if new_text:
                    # Simple replacement for text in fields
                    t_elem.text = new_text
                    stats['replacements'] += 1

def process_paragraph(para, stats):
    """Process paragraph including hyperlinks."""
    ...
    # Process hyperlinks (for TOC entries)
    ns = {'w': 'http://schemas.openxmlformats.org/wordprocessingml/2006/main'}
    hyperlinks = para._element.findall('.//w:hyperlink', ns)
    for hyperlink in hyperlinks:
        process_runs_in_element(hyperlink, stats)
...
def process_document(docx_path):
    ...
    stats = {'replacements': 0, 'font_fixes': 0}

    # Process all paragraphs (including TOC)
...
print("Updating Shabuw'ah and Taruw'ah including TOC hyperlinks...")
print()
