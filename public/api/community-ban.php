...
if ($method === 'GET') {
    $stmt = $db->prepare("SELECT user_banned_flag, user_banned_until, user_ban_reason, user_muted_flag, user_muted_until, user_display_name FROM yy_user WHERE user_key = ?");
    $stmt->execute([$targetKey]);
    $user = $stmt->fetch();
