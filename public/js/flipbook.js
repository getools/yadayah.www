...
// POST batched flipbook errors to the same endpoint the rest of the
// site uses (/api/client-error.php). They land in yy_monitor_event
// alongside js_error rows from the global error-reporter, so they
// show up in /admin-monitor.html with no extra wiring. Wrap in
// try/catch + an outer `try` per call — never throw from the error
// path or we'll feedback-loop the handler.
const reportFlipbookError = (payload) => {
  try {
...
window.onerror = (msg, src, line, col, e) => {
  // Cross-origin censored errors come back as the literal string
  // "Script error." with no source/line/col. Browsers redact these
  // when they originate inside a third-party iframe or external
  // script (the page-flip CDN, fullscreen internals, etc.) — there's
  // nothing actionable to display or store, and surfacing the
  // redaction would just spam the monitor.
  if (/^Script error\.?$/i.test(msg) && !src) return;
  showErr(`onerror: ${msg} @${src}:${line}:${col}\n${e && e.stack ? e.stack : ''}`);
  reportFlipbookError({
    type:    'js_error',
    message: 'flipbook: ' + (msg || '') + '',
    source:  (src  || '') + '',
    lineno:  line  || 0,
    colno:   col   || 0,
...
window.onunhandledrejection = ev => {
  ...
  showErr('unhandled rejection: ' + (reason.stack || message));
  reportFlipbookError({
    type:    'promise_rejection',
    message: 'flipbook: ' + message,
...
window.addEventListener('load', async () => {
  try {
    ...
    // Read per-book config from the shell page (window.FLIPBOOK_CONFIG).
    // Required: bookCode + pageCount. title is optional and falls back
    // to a humanised version of bookCode for the toolbar h1.
    const cfg = window.FLIPBOOK_CONFIG || {};
    if (!cfg.bookCode)  { showErr('FLIPBOOK_CONFIG.bookCode not set on page'); return; }
    if (!cfg.pageCount) { showErr('FLIPBOOK_CONFIG.pageCount not set on page'); return; }

    const TOTAL = cfg.pageCount | 0;
    const PAD = n => String(n).padStart(3, '0');
    const PAGE_RATIO = 9 / 6;   // 6"×9" paperback — matches the source PDF dimensions
    const BOOK_CODE  = cfg.bookCode;
    const PDF_PATH   = '/pdf/' + BOOK_CODE + '.pdf';
    const STORE_KEY  = 'flipbook:' + BOOK_CODE;

    // Apply config-driven chrome BEFORE any async work so the user sees
    // the right title/meta during the brief asset-load window.
    const _title = cfg.title || BOOK_CODE.replace(/^YY-s\d{2}v\d{2}-/, '').replace(/-/g, ' ');
    document.title = _title + ' · Flipbook';
    const _h1 = document.querySelector('header h1');
    if (_h1)   _h1.textContent = _title;
    const _meta = document.querySelector('header .meta');
    if (_meta) _meta.textContent = 'Self-hosted flipbook · ' + TOTAL + ' pages · drag any text to select · click far edges or top corners to turn';
    const _seek = document.getElementById('seek');
    if (_seek) _seek.max = String(TOTAL);
    const _ptotal = document.getElementById('ptotal');
    if (_ptotal) _ptotal.textContent = String(TOTAL);
