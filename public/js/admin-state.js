/* admin-state.js — generic auto-persistence for admin UI state
   ============================================================
   Drop this script into any admin page and it will:
     - Bind every <input>, <select>, <textarea> with an `id`
     - Save its value to localStorage AND /api/admin-state.php on change
   ...
   Server sync:
     - Loads from /api/admin-state.php?scope=<page> on init, then layers
   ...
   Public surface (window.adminState):
...
(function () {
    ...
    var STORAGE_PREFIX = 'admin-state:' + SCOPE + ':';
    var API_URL = '/api/admin-state.php';
    var DEBOUNCE_MS = 300;
    ...
    window.adminState = { save: save, load: load, bind: bindEl, scope: SCOPE, clear: clearAll };
})();
