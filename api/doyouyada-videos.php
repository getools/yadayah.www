...
if ($limit > 0) {
    $stmt = $db->prepare("SELECT feed_item_key AS feed_key, feed_item_external_id AS feed_video_id, TRIM(BOTH '~ -' FROM TRIM(REGEXP_REPLACE(feed_item_title, '#\w+\s*', '', 'g'))) AS feed_title, feed_item_thumbnail AS feed_thumbnail, feed_item_publish_dtime AS feed_create, feed_item_type AS feed_type, feed_item_url AS feed_url FROM yy_feed_item WHERE $where ORDER BY feed_item_publish_dtime DESC NULLS LAST LIMIT ?");
...
$stmt = $db->prepare("
    SELECT feed_item_key AS feed_key, feed_item_external_id AS feed_video_id,
           ...
           feed_item_publish_dtime AS feed_create, feed_item_type AS feed_type, feed_item_url AS feed_url
    FROM yy_feed_item
    WHERE $where
